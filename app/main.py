...
Endpoints:
  POST /run-experiment      – Single or counterfactual (Run A + Run B) experiment.
  POST /run-monte-carlo     – N repeated trials for statistical significance.
  GET  /experiment/{id}     – Full logged JSON for a past experiment.
  GET  /tasks               – Available task descriptions.
  GET  /health
"""
...
from app.config import EXPERIMENTS_DIR, TASKS
from app.experiment import run_single_experiment, run_monte_carlo
from app.metrics.analyzer import compare_runs
...
app = FastAPI(
    ...
    description=(
        "Demonstrates structural dominance bias in multi-agent LLM deliberation. "
        "A dominant agent (higher token budget, speaks first) measurably biases peer agents "
        "toward its framing through structural authority alone."
    ),
    version="2.0.0",
)
...
class ExperimentRequest(BaseModel):
    """
    Parameters for a single or counterfactual Context Corrosion run.

    task_id           : 0–4
    temperature       : shared across all agents
    rounds            : discussion rounds (1–5)
    biased_dominant   : inject biased framing into D (Run A) or not (Run B)
    run_counterfactual: automatically run both biased=True and biased=False and compare
    """
    ...
    run_counterfactual: bool = Field(
        default=False,
        description="Run both biased and unbiased and return statistical comparison",
    )


class MonteCarloRequest(BaseModel):
    """
    Parameters for a Monte Carlo repeated-trial run.

    n_trials : number of independent replications (2–20)
    All other fields same as ExperimentRequest.
    Runs n_trials for biased=True AND n_trials for biased=False, then
    computes Welch's t-test + Cohen's d across the two sets.
    """
    task_id: int = Field(default=0, ge=0, le=4)
    temperature: float = Field(default=0.7, ge=0.0, le=1.0)
    rounds: int = Field(default=3, ge=1, le=5)
...
async def run_experiment_endpoint(req: ExperimentRequest) -> dict[str, Any]:
    """
    Run a Context Corrosion experiment (single or counterfactual).

    If run_counterfactual=True:
      Executes Run A (biased=True) and Run B (biased=False), returns comparison.
    """
    try:
        if req.run_counterfactual:
            result_a = run_single_experiment(req.task_id, req.temperature, req.rounds, True)
            result_b = run_single_experiment(req.task_id, req.temperature, req.rounds, False)
            comparison = compare_runs(result_a["metrics"], result_b["metrics"])
            return {
                "mode": "counterfactual",
                "run_A": {
        ...
        result = run_single_experiment(
            req.task_id, req.temperature, req.rounds, req.biased_dominant
        )
        return {
            "mode": "single",
            "experiment_id": result["experiment_id"],
...
async def run_monte_carlo_endpoint(req: MonteCarloRequest) -> dict[str, Any]:
    """
    Run n_trials of both biased (A) and unbiased (B) variants.
    Returns aggregate statistics, Welch's t-test p-values, and Cohen's d
    for all key metrics — enabling statistically validated corrosion claims (Issue 6).
    """
    try:
        mc_a = run_monte_carlo(req.task_id, req.temperature, req.rounds, True, req.n_trials)
        mc_b = run_monte_carlo(req.task_id, req.temperature, req.rounds, False, req.n_trials)
        ...
        logger.exception("Monte Carlo run failed")
        raise HTTPException(status_code=500, detail=f"Monte Carlo failed: {e}")


@app.get("/experiment/{experiment_id}")
async def get_experiment(experiment_id: str) -> dict[str, Any]:
    """Retrieve full logged JSON for a completed experiment."""
    return _load_experiment(experiment_id)


@app.get("/tasks")
async def list_tasks() -> dict[str, Any]:
    """List all available ambiguous reasoning task templates."""
    return {
